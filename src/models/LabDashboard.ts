  ...
  const stats = await LabTestRequest.aggregate([
    ...
    },
    {
      $facet: {
        ...
        pendingTests: [
          { $match: { status: { $in: ["REQUESTED", "SAMPLE_COLLECTED", "IN_PROGRESS"] } } },
          { $count: "count" }
        ],
        criticalTests: [
...
const LabDashboard = models.LabDashboard || model<ILabDashboardDocument>("LabDashboard", LabDashboardSchema);
