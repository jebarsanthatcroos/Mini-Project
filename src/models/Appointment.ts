...
export interface IAppointmentModel extends Model<IAppointmentDocument> {
  findByPharmacist(pharmacistId: Types.ObjectId, filters?: any): Promise<IAppointmentDocument[]>;
  findByDateRange(startDate: Date, endDate: Date, pharmacistId?: Types.ObjectId): Promise<IAppointmentDocument[]>;
  findUpcomingByPharmacist(pharmacistId: string, limit?: number): Promise<IAppointmentDocument[]>;
  ...
  getStats(pharmacistId: Types.ObjectId, startDate: Date, endDate: Date): Promise<any[]>;
}
...
const AppointmentSchema = new Schema<IAppointmentDocument, IAppointmentModel>(
  {
    ...
    serviceType: {
      type: String,
      enum: ["MEDICATION_REVIEW", "CHRONIC_DISEASE_MANAGEMENT", "VACCINATION", "HEALTH_SCREENING", "PRESCRIPTION_CONSULTATION", "OTHER"],
...
// Virtuals
AppointmentSchema.virtual('isUpcoming').get(function(this: IAppointmentDocument) {
  const now = new Date();
...
AppointmentSchema.virtual('timeSlot').get(function(this: IAppointmentDocument) {
  return `${this.appointmentTime} - ${new Date(this.appointmentDate).toLocaleDateString()}`;
});

// Methods
AppointmentSchema.methods.canBeCancelled = function(this: IAppointmentDocument): boolean {
  const cancellableStatuses = ["SCHEDULED", "CONFIRMED"];
  return cancellableStatuses.includes(this.status);
};

AppointmentSchema.methods.canBeRescheduled = function(this: IAppointmentDocument): boolean {
  const reschedulableStatuses = ["SCHEDULED", "CONFIRMED"];
  return reschedulableStatuses.includes(this.status);
};

AppointmentSchema.methods.updateStatus = function(this: IAppointmentDocument, newStatus: AppointmentStatus): Promise<IAppointmentDocument> {
  this.status = newStatus;
  return this.save();
};

AppointmentSchema.methods.reschedule = function(this: IAppointmentDocument, newDate: Date, newTime: string): Promise<IAppointmentDocument> {
...
AppointmentSchema.methods.addPrescriptionRefill = function(this: IAppointmentDocument, refillData: any): void {
...
AppointmentSchema.methods.recordVitalSigns = function(this: IAppointmentDocument, vitals: any): void {
  this.vitalSigns = { ...this.vitalSigns, ...vitals };
};

// Static Methods
AppointmentSchema.statics.findByPharmacist = function(pharmacistId: Types.ObjectId, filters: any = {}) {
...
AppointmentSchema.statics.findByDateRange = function(startDate: Date, endDate: Date, pharmacistId?: Types.ObjectId) {
  const query: any = {
...
AppointmentSchema.statics.findUpcomingByPharmacist = function(pharmacistId: string, limit: number = 10) {
  const now = new Date();
  return this.find({
...
AppointmentSchema.statics.findTodayByPharmacist = function(pharmacistId: string) {
  const today = new Date();
...
AppointmentSchema.statics.getStats = function(pharmacistId: Types.ObjectId, startDate: Date, endDate: Date) {
...
const Appointment = (models.Appointment as IAppointmentModel) || 
  model<IAppointmentDocument, IAppointmentModel>("Appointment", AppointmentSchema);
