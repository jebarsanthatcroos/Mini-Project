...
export interface IDoctorModel extends Model<IDoctor> {
  ...
  findAvailableDoctors(day: string, time: string, limit?: number): Promise<IDoctor[]>;
...
DoctorSchema.methods.isAvailable = function(this: IDoctor, date: Date, time: string): boolean {
  ...
  const [startHour, startMinute] = this.profile.availability.startTime.split(':').map(Number);
  const [endHour, endMinute] = this.profile.availability.endTime.split(':').map(Number);
...
DoctorSchema.methods.getUpcomingAppointments = async function(this: IDoctor, limit: number = 10): Promise<any[]> {
...
DoctorSchema.methods.getPatientStatistics = async function(this: IDoctor): Promise<{
  totalPatients: number;
  appointmentsThisMonth: number;
  averageRating: number;
}> {
  ...
  const [totalPatients, appointmentsThisMonth] = await Promise.all([
    Appointment.distinct('patient', { doctor: this._id }).then(patients => patients.length),
...
DoctorSchema.statics.findBySpecialization = function(specialization: string, limit: number = 10) {
  return this.find({ 
...
DoctorSchema.statics.findAvailableDoctors = function(day: string, time: string, limit: number = 10) {
  return this.find({
...
const Doctor = models.Doctor || model<IDoctor, IDoctorModel>("Doctor", DoctorSchema);
