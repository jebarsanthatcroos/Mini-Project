...
export default function PharmacistAppointmentsPage() {
  ...
  const updateAppointmentStatus = async (appointmentId: string, newStatus: string) => {
    try {
      const response = await fetch(`/api/pharmacy/appointments/${appointmentId}/status`, {
        method: 'PATCH',
        headers: {
      ...
      if (data.success) {
        setAppointments(prev => prev.map(apt => 
          apt.id === appointmentId ? data.data : apt
        ));
      } else {
        ...
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-2">Manage patient consultations and services</p>
            </div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => router.push('/Pharmacist/appointments/appointment')}
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              >
                ...
                <option value="CHRONIC_DISEASE_MANAGEMENT">Chronic Disease</option>
              ...
              <button
                onClick={() => setFilters({ status: 'all', date: '', search: '', serviceType: 'all' })}
          ...
          <AnimatePresence>
            {appointments.map((appointment, index) => (
              ...
              >
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {getPatientAge(appointment)} yrs • {appointment.patient.userId.gender.toLowerCase()}
                      </p>
                      <p className="text-xs text-gray-500 font-mono mt-1">
                        MRN: {appointment.patient.medicalRecordNumber}
                      </p>
                    </div>
                    <span className={`px-3 py-1 rounded-full text-xs font-medium border ${statusConfig[appointment.status as keyof typeof statusConfig]?.color || 'bg-gray-100 text-gray-800'}`}>
                ...
                <div className="p-6">
                  <div className="mb-4">
                    <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium mb-3 ${serviceTypeColors[appointment.serviceType as keyof typeof serviceTypeColors] || 'bg-gray-100 text-gray-800'}`}>
                      {serviceTypeLabels[appointment.serviceType as keyof typeof serviceTypeLabels] || appointment.serviceType}
                    </span>
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
                    ...
                    <span className="truncate">{appointment.pharmacy.name}</span>
                  </div>
        ...
        {appointments.length === 0 && (
          ...
          >
            ...
            <p className="text-gray-600 mb-6">
              {filters.status !== 'all' || filters.date || filters.search || filters.serviceType !== 'all'
