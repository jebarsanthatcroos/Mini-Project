...
import { FiArrowLeft, FiEdit, FiSave, FiX, FiUpload, FiPackage, FiAlertCircle } from 'react-icons/fi';
...
export default function ProductDetailPage() {
  ...
  const categories = ['Prescription', 'OTC', 'Supplements', 'Medical Devices', 'Personal Care', 'First Aid', 'Baby Care', 'Vitamins', 'Other'];
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (type === 'checkbox') {
      ...
      setEditData(prev => ({ ...prev, [name]: value === '' ? '' : Number(value) }));
    } else {
  ...
  const stockStatus = !product.inStock || product.stockQuantity === 0 
    ? { text: 'Out of Stock', color: 'text-red-600 bg-red-50 border-red-200' }
    : product.stockQuantity <= product.minStockLevel
    ? { text: 'Low Stock', color: 'text-yellow-600 bg-yellow-50 border-yellow-200' }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            <motion.div
              ...
              className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm p-6 sticky top-6 border border-white/20"
            >
              <h2 className="text-lg font-semibold mb-4 text-gray-900">Product Image</h2>
              
              {isEditing ? (
                <div className="space-y-4">
                  <div className="relative h-64 bg-linear-to-br from-gray-50 to-gray-100 rounded-xl overflow-hidden">
                    <Image
                      src={imagePreview || editData.image || '/placeholder-product.jpg'}
              ...
              <div className="mt-6 space-y-3">
                ...
                <div className="flex justify-between items-center pb-3 border-b border-gray-200">
                  ...
                  <span className="font-medium text-gray-900">{product.pharmacy.name}</span>
                </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.div
              className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm p-6 border border-white/20"
            >
              <h2 className="text-xl font-semibold mb-4 text-gray-900">Pricing & Stock</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-2xl font-bold text-green-600">Rs. {product.price.toFixed(2)}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-gray-900 text-lg">Rs. {product.costPrice.toFixed(2)}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-gray-900 text-lg">{product.stockQuantity} units</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-gray-900 text-lg">{product.minStockLevel} units</p>
                  )}
                </div>
                ...
                {isEditing && (
                  <div className="flex items-center md:col-span-2 p-3 bg-blue-50/50 rounded-xl border border-blue-200">
                    ...
                    />
                    <label className="ml-2 text-sm text-gray-700 font-medium">Product is in stock</label>
                  </div>
                )}
              </div>
            </motion.div>
            ...
            <motion.div
              className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm p-6 border border-white/20"
            >
              <h2 className="text-xl font-semibold mb-4 text-gray-900">Medical Information</h2>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Active Ingredients</label>
                  {isEditing ? (
                    <input
                      type="text"
                    ...
                    <p className="text-gray-900">{product.activeIngredients || 'Not specified'}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-gray-900">{product.dosage || 'Not specified'}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-gray-900">{product.sideEffects || 'Not specified'}</p>
                  )}
                </div>
                  ...
                  <div className="flex items-center p-3 bg-purple-50/50 rounded-xl border border-purple-200">
                    {isEditing ? (
                      <>
                        ...
                        />
                        <label className="ml-2 text-sm text-gray-700 font-medium">Requires prescription</label>
                      </>
                    ) : (
                      <>
                        <span className="text-gray-600">Prescription:</span>
                        <span className={`ml-2 font-medium ${product.requiresPrescription ? 'text-purple-600' : 'text-gray-900'}`}>
                  ...
                  <div className="flex items-center p-3 bg-orange-50/50 rounded-xl border border-orange-200">
                    {isEditing ? (
                      <>
                        ...
                        />
                        <label className="ml-2 text-sm text-gray-700 font-medium">Controlled substance</label>
                      </>
                    ) : (
                      <>
                        <span className="text-gray-600">Controlled:</span>
                        <span className={`ml-2 font-medium ${product.isControlledSubstance ? 'text-orange-600' : 'text-gray-900'}`}>
