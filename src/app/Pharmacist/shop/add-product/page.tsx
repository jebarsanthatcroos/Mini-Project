...
import { FiArrowLeft, FiSave, FiUpload, FiAlertCircle, FiCheck, FiX } from 'react-icons/fi';
...
const productSchema = z.object({
  ...
  price: z.string().min(1, 'Price is required').refine((val) => parseFloat(val) > 0, 'Price must be greater than 0'),
  costPrice: z.string().optional().or(z.literal('')),
  category: z.string().min(1, 'Category is required'),
  image: z.string().min(1, 'Product image is required'),
  inStock: z.boolean().default(true),
  stockQuantity: z.string().min(1, 'Stock quantity is required').refine((val) => parseInt(val) >= 0, 'Stock quantity cannot be negative'),
  ...
  const onSubmit: SubmitHandler<ProductFormData> = async (data) => {
    try {
      ...
      const payload = {
        ...
        costPrice: data.costPrice ? parseFloat(data.costPrice as unknown as string) : 0,
      ...
      >
        ...
        <motion.div variants={itemVariants} className="mb-6">
          ...
          >
            ...
            <span className="group-hover:text-blue-600 transition-colors">Back to Shop</span>
          </button>
          <div className="text-center">
            <h1 className="text-4xl font-bold bg-linear-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
              Add Product
            </h1>
            <p className="text-gray-600 mt-2">Add a new product to your pharmacy inventory</p>
          </div>
        </motion.div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 sticky top-6 border border-white/20">
                ...
                <div className="space-y-4">
                  <div className="relative h-80 bg-linear-to-br from-gray-50 to-gray-100 rounded-xl overflow-hidden border-2 border-dashed border-gray-300/50 hover:border-blue-400/50 transition-all duration-300 group">
                    {imagePreview ? (
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 group-hover:text-blue-400 transition-colors duration-300">
                        ...
                        <p className="text-center text-sm">Click to upload<br/>product image</p>
                      </div>
                    )}
                  </div>
                  ...
                  <label className="cursor-pointer flex items-center justify-center gap-3 w-full px-4 py-4 border-2 border-dashed border-gray-300/50 rounded-xl hover:border-blue-500 hover:bg-blue-50/50 transition-all duration-300 group">
                    ...
                    <span className="text-blue-600 font-medium">Choose Product Image</span>
                    <input
                      type="file"
                      accept="image/*"
            ...
            <div className="lg:col-span-2 space-y-6">
              {/* Basic Information */}
              <motion.div variants={itemVariants} className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
              ...
              <motion.div variants={itemVariants} className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    />
                    <label className="text-sm font-medium text-gray-700">Product is in stock</label>
                  </div>
                </div>
              </motion.div>
